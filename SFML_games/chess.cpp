...
const array<array<vector<int>,4>,64> chess::init_bishopAtks(){
    ...
    for( unsigned int c_id = 0; c_id < 8; c_id++ ){
        ...
        ind_a = r_id * chess::BOARDWIDTH + c_id;

        // North-East diagonal scan.
        r_z = r_id + 1;     c_z = c_id + 1;
        while( r_z < 8 && c_z < 8 ){
            res_arr[ind_a][0].push_back( r_z * chess::BOARDWIDTH + c_z );
            r_z++;  c_z++;
        }
        ...
        r_z = r_id + 1;     c_z = c_id - 1;
        while( r_z < 8 && c_z >= 0 ){
            res_arr[ind_a][1].push_back( r_z * chess::BOARDWIDTH + c_z );
            r_z++;  c_z--;
        }
        ...
        r_z = r_id - 1;     c_z = c_id - 1;
        while( r_z >= 0 && c_z >= 0 ){
            res_arr[ind_a][2].push_back( r_z * chess::BOARDWIDTH + c_z );
            r_z--;  c_z--;
        }
        ...
        r_z = r_id - 1;     c_z = c_id + 1;
        while( r_z >= 0 && c_z < 8 ){
            res_arr[ind_a][3].push_back( r_z * chess::BOARDWIDTH + c_z );
            r_z--;  c_z++;
        }
...
const array<array<vector<int>,4>,64> chess::rookAtks = chess::init_rookAtks();

const array<array<vector<int>,4>,64> chess::init_rookAtks(){
...
// ---------------------------------------------------------------------- >>>>>
    ...
    for( chs_move move_z : this->en_pass_moves ){
        ...
        int pt_a_ind = chess::sub2ind( move_z.pt_a );
        int pt_b_ind = chess::sub2ind( move_z.pt_b );
        int pt_c_ind = chess::sub2ind( move_z.pt_a.first, move_z.pt_b.second );
        ...
        if( pt_b_ind > pt_a_ind )
        {
            ...
            this->atk_list_by_W[ pt_b_ind ].erase(
                ...
                this->atk_list_by_W[ pt_a_ind ].end() );
            this->valid_W_atks_map[ pt_a_ind ].erase(
                std::remove(this->valid_W_atks_map[ pt_b_ind ].begin(), 
                this->valid_W_atks_map[ pt_b_ind ].end(), pt_a_ind ), 
                this->valid_W_atks_map[ pt_b_ind ].end() );
        ...
        // Black move (goes down).
            ...
            this->atk_list_by_B[ pt_b_ind ].erase(
                ...
                this->atk_list_by_B[ pt_a_ind ].end() );
            this->valid_B_atks_map[ pt_a_ind ].erase(
                std::remove(this->valid_B_atks_map[ pt_b_ind ].begin(), 
                this->valid_B_atks_map[ pt_b_ind ].end(), pt_a_ind ), 
                this->valid_B_atks_map[ pt_b_ind ].end() );
...
// ---------------------------------------------------------------------- >>>>>
    ...
    if( tar_pce.type == CHS_PIECE_TYPE::KNIGHT ){
        ...
        for( unsigned int z = 0; z < 4; z++ ){
            for( int ind_zz : chess::rookAtks[ind_a][z] ){
                tmp_ind_arr[tmp_arr_lim++] = ind_zz;
            }
        }            
...
// ---------------------------------------------------------------------- >>>>>
//      Removal of Points Affected by Previous Piece
// ---------------------------------------------------------------------- >>>>>
...
void chess::upd_pre_legal_plays( const int ind_a, const int ind_b, const chs_piece prev_pce ){
    ...
    // Reset the destination square to its previous occupant for scanning.
    this->CHS_board[i_b][j_b] = prev_pce;
...
*/
    ...
    }else if( i_a == chess::BOARDHEIGHT - 1 && this->CHS_board[7][4].type == CHS_PIECE_TYPE::KING &&
    ...
    }else if( i_b == chess::BOARDHEIGHT - 1 && this->CHS_board[7][4].type == CHS_PIECE_TYPE::KING &&
        this->CHS_board[7][4].not_moved )
    {
        ...
        {
            // Remove right-side castling possibility.
            this->valid_B_moves_map[60].erase(
                std::remove(this->valid_B_moves_map[60].begin(), 
                this->valid_B_moves_map[60].end(), 62 ), 
                this->valid_B_moves_map[60].end() );
            ...
            this->CHS_board[7][0].not_moved )
        {
            ...
            this->valid_B_moves_map[60].erase(
                std::remove(this->valid_B_moves_map[60].begin(), 
                this->valid_B_moves_map[60].end(), 58 ), 
                this->valid_B_moves_map[60].end() );
        }
...
void chess::upd_pre_legal_promo( const bool is_w, const unsigned int c_idx ){

    if( c_idx >= chess::BOARDWIDTH ){
        throw std::invalid_argument( "Column index out of bound." );
    }
    ...
    if( this->CHS_board[r_idx][c_idx].type == CHS_PIECE_TYPE::NO_P || 
        this->CHS_board[r_idx][c_idx].type == CHS_PIECE_TYPE::PAWN ||
        this->CHS_board[r_idx][c_idx].type == CHS_PIECE_TYPE::KING )
    {
