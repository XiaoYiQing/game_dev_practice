...
string chess::bestMove( int depth ){
    ...
    string optimComm = IMPOS_ALG_COMM;

    if( this->state == CHS_STATE::WWIN || this->state == CHS_STATE::BWIN || 
    ...
    // Thread exit point.
    if( !this->AI_proc_flag ){
        return IMPOS_ALG_COMM;
    }

    // Obtain the entire set of currently valid moves.
    vector <string> validCommVec = this->get_all_psbl_alg_comm();

    for( unsigned int z = 0; z < validCommVec.size(); z++ ){

        // Current valid move.
        string comm_z = validCommVec.at(z);
    
        // Make a copy of the current game.
        chess newGame = *this;

        // Perform the current play.
        newGame.ply_ag_comm( comm_z );
...
string chess::bestMove_ABP( int depth ){
